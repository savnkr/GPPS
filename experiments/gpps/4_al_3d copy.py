...
def sdd_poisson_solver_3d(
    ...
    for epoch in range(n_epochs):
        ...
        if n_batches > 0:
            ...
            if epoch % 100 == 0:
                print(f"Epoch {epoch}: Loss = {avg_loss:.6f}, Lengthscale = {kernel.lengthscale.item():.4f}, Variance = {kernel.variance.item():.4f}")
...
def visualize_3d_solution(X_test, posterior_mean, actual_solution, error, posterior_std, title_suffix="", save_path=None):
    """
    Visualize the 3D solution with full 3D scatter plots.
    
    Args:
        X_test: Test points [N, 3]
        posterior_mean: Predicted mean values [N, 1]
        actual_solution: True solution values [N]
        error: Absolute error [N]
        posterior_std: Posterior standard deviation [N]
        title_suffix: Text to add to plot titles
    ...
    if isinstance(posterior_mean, torch.Tensor):
        posterior_mean_np = posterior_mean.cpu().numpy().reshape(-1)
    else:
        posterior_mean_np = posterior_mean
        
    if isinstance(posterior_std, torch.Tensor):
        posterior_std_np = posterior_std.cpu().numpy().reshape(-1)
    else:
        posterior_std_np = posterior_std
    
    # Create 3D subplots
    fig, axes = plt.subplots(2, 2, figsize=(15, 12), subplot_kw={'projection': '3d'})
    
    # Plot the actual solution (ground truth)
    scat1 = axes[0, 0].scatter(x1, x2, x3, c=actual_solution, cmap='viridis', )
    fig.colorbar(scat1, ax=axes[0, 0])
    ...
    scat2 = axes[0, 1].scatter(x1, x2, x3, c=posterior_mean_np, cmap='viridis', )
    fig.colorbar(scat2, ax=axes[0, 1])
    axes[0, 1].set_title(f'Mean Prediction {title_suffix}')
    axes[0, 1].set_xlabel('$x_1$')
    axes[0, 1].set_ylabel('$x_2$')
    axes[0, 1].set_zlabel('$x_3$')
    
    # Plot the error
    scat3 = axes[1, 0].scatter(x1, x2, x3, c=error, cmap='coolwarm', )
    fig.colorbar(scat3, ax=axes[1, 0])
    ...
    scat4 = axes[1, 1].scatter(x1, x2, x3, c=posterior_std_np, cmap='plasma', )
    fig.colorbar(scat4, ax=axes[1, 1])
    axes[1, 1].set_title(f'Standard Deviation')
...
def save_results(X_test, actual_solution, posterior_mean_sdd, posterior_std_sdd, sdd_error,
                ...
                convergence_data, path='poisson_3d_results.npz'):
    """
...
# Added a run call for the new experiment
def random_sampling_3d(X_pool, X_train, na=5, exclusion_radius=0.05):
    """
    Random sampling baseline for comparison with active learning.
    ...
    Returns:
        Randomly selected points to add to training set
    """
    # Filter candidate pool - remove points too close to training set
    X_pool_filtered = filter_candidates_3d(X_pool, X_train, exclusion_radius)
    ...
    # Randomly select na points from filtered pool
    n_available = X_pool_filtered.shape[0]
    n_select = min(na, n_available)
    
    # Random selection without replacement
    indices = torch.randperm(n_available)[:n_select]
    return X_pool_filtered[indices]

def run_3d_comparison_experiment_with_random_baseline(n_initial=6, n_final=50, n_iterations=8, n_test=500):
    """
    Run experiments comparing active learning methods with random sampling baseline.
    Uses fewer initial points (5-6) to better demonstrate active learning benefits.
    """
    # Parameters
    n_per_iter = (n_final - n_initial) // n_iterations
    ...
    # Create a large candidate pool within the unit cuboid
    X_pool = sobol_cuboid_sampling(2000, domain_bounds=(0.05, 0.95))
    ...
    # Generate initial points using Sobol sequence (same for all methods)
    Xi_initial = sobol_cuboid_sampling(n_initial, domain_bounds=(0.05, 0.95))
    f_Xi_initial = -3 * (np.pi ** 2) * torch.sin(np.pi * Xi_initial[:, 0]) * torch.sin(np.pi * Xi_initial[:, 1]) * torch.sin(np.pi * Xi_initial[:, 2])
    f_Xi_initial = f_Xi_initial.reshape(-1, 1).to(torch.float64)
    
    # ------- RANDOM SAMPLING BASELINE -------
    Xi_random = Xi_initial.clone()
    f_Xi_random = f_Xi_initial.clone()
    
    # Lists to track metrics over iterations
    random_points = [n_initial]
    random_errors = []
    random_variances = []
    
    # Initial prediction with random points
    C_full_random = solver.compute_covariance_matrix(Xi_random, Xb)
    C_inv_random = torch.inverse(C_full_random + 1e-6 * torch.eye(C_full_random.shape[0]))
    y_obs_random = torch.cat((f_Xi_random, g_Xb), dim=0)
    
    posterior_mean_random = solver.posterior_mean(X_test, Xi_random, Xb, C_inv_random, y_obs_random).detach()
    random_abs_error = np.abs(posterior_mean_random.cpu().numpy().reshape(-1) - actual_solution)
    random_errors.append(np.mean(random_abs_error))
    
    # Calculate posterior variance
    cov_full_random = solver.posterior_covariance(X_test, X_test, Xi_random, Xb, C_inv_random).detach()
    variance_random = torch.diag(cov_full_random).cpu().numpy().reshape(-1)
    random_variances.append(np.mean(variance_random))
    ...
    for it in range(n_iterations):
        ...
        # ----- Random Sampling -----
        new_samples_random = random_sampling_3d(
            X_pool=X_pool,
            X_train=Xi_random,
            na=n_per_iter,
            exclusion_radius=0.05
        ...
        if new_samples_random.shape[0] > 0:
            # Add new points to training set
            Xi_random = torch.cat([Xi_random, new_samples_random], dim=0)
            
            # Compute values at new points
            f_Xi_random = -3 * (np.pi ** 2) * torch.sin(np.pi * Xi_random[:, 0]) * torch.sin(np.pi * Xi_random[:, 1]) * torch.sin(np.pi * Xi_random[:, 2])
            f_Xi_random = f_Xi_random.reshape(-1, 1).to(torch.float64)
        ...
        # Track number of points
        random_points.append(Xi_random.shape[0])
        no_cluster_points.append(Xi_no_cluster.shape[0])
        with_cluster_points.append(Xi_with_cluster.shape[0])
        
        print(f"  Random Sampling - Points: {Xi_random.shape[0]}, Error: {random_errors[-1]:.6f}, Variance: {random_variances[-1]:.6f}")
        print(f"  AL without Clustering - Points: {Xi_no_cluster.shape[0]}, Error: {no_cluster_errors[-1]:.6f}, Variance: {no_cluster_variances[-1]:.6f}")
        print(f"  AL with Clustering - Points: {Xi_with_cluster.shape[0]}, Error: {with_cluster_errors[-1]:.6f}, Variance: {with_cluster_variances[-1]:.6f}")
    
    print("\n--- Final Results ---")
    print(f"Random Sampling ({Xi_random.shape[0]} points): Mean Error = {random_errors[-1]:.6f}")
    print(f"AL without Clustering ({Xi_no_cluster.shape[0]} points): Mean Error = {no_cluster_errors[-1]:.6f}")
    print(f"AL with Clustering ({Xi_with_cluster.shape[0]} points): Mean Error = {with_cluster_errors[-1]:.6f}")
    
    # Calculate improvement percentages
    random_final_error = random_errors[-1]
    no_cluster_improvement = ((random_final_error - no_cluster_errors[-1]) / random_final_error) * 100
    with_cluster_improvement = ((random_final_error - with_cluster_errors[-1]) / random_final_error) * 100
    
    print(f"\nImprovement over Random Sampling:")
    print(f"AL without Clustering: {no_cluster_improvement:.2f}% reduction in error")
    print(f"AL with Clustering: {with_cluster_improvement:.2f}% reduction in error")
    
    # ------- VISUALIZATION -------
    # Plot 1: Error convergence comparison
    plt.figure(figsize=(12, 8))
    plt.plot(random_points, random_errors, 'o-', color='red', linewidth=2, label='Random Sampling (Baseline)')
    plt.plot(no_cluster_points, no_cluster_errors, 'o-', color='blue', linewidth=2, label='AL without Clustering')
    plt.plot(with_cluster_points, with_cluster_errors, 'o-', color='green', linewidth=2, label='AL with Clustering')
    plt.xlabel('Number of Training Points')
    plt.ylabel('Mean Absolute Error')
    plt.title('Active Learning vs Random Sampling (3D Poisson Equation)')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig('al_vs_random_error_convergence_3d.png', dpi=300, bbox_inches='tight')
    
    # Plot 2: Variance convergence comparison
    plt.figure(figsize=(12, 8))
    plt.plot(random_points, random_variances, 'o-', color='red', linewidth=2, label='Random Sampling (Baseline)')
    plt.plot(no_cluster_points, no_cluster_variances, 'o-', color='blue', linewidth=2, label='AL without Clustering')
    plt.plot(with_cluster_points, with_cluster_variances, 'o-', color='green', linewidth=2, label='AL with Clustering')
    plt.xlabel('Number of Training Points')
    plt.ylabel('Mean Posterior Variance')
    plt.title('Uncertainty Reduction: Active Learning vs Random Sampling')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig('al_vs_random_variance_convergence_3d.png', dpi=300, bbox_inches='tight')
    
    return {
        'random_error': random_errors[-1],
        'no_cluster_error': no_cluster_errors[-1],
        'with_cluster_error': with_cluster_errors[-1],
        'no_cluster_improvement': no_cluster_improvement,
        'with_cluster_improvement': with_cluster_improvement,
        'convergence': {
...
if __name__ == "__main__":
    # Run the comparison with random baseline
    run_3d_comparison_experiment_with_random_baseline(n_initial=5, n_final=50, n_iterations=8)
